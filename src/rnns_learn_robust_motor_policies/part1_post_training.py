...
if __name__ == '__main__':
    ...
    model_filenames = [
        filename for filename in os.listdir(args.directory)
        if args.file_pattern in filename
    ]
    
    for filename in model_filenames:
        models, model_hyperparameters = load_with_hyperparameters(
            filename, setup_models,
        )
        
        train_histories = load(
            filename.replace('trained_models', 'train_histories'),
            partial(setup_train_histories, task_base, models),
        )
        
        hyperparameters = load_from_json(
            filename.replace('trained_models.eqx', 'hyperparameters.json')
        )
        where_train = attr_str_tree_to_where_func(hyperparameters['where_train_strs'])
        n_replicates = hyperparameters['n_replicates']        
        save_model_parameters = hyperparameters['save_model_parameters']
        ...
        write_to_json(
            ...
            filename.replace('trained_models.eqx', 'extras.json'),
        )
        ...
        save(
            filename.replace(args.file_pattern, args.file_pattern.replace('.eqx', '_best_params.eqx')),
