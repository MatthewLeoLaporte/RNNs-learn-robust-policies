...
from rnns_learn_robust_motor_policies.analysis.activity import NetworkActivities_SampleUnits
...
COLOR_FUNCS = dict(
    pert__amp=lambda hps: (0, hps.pert.context.c_final - hps.pert.context.c_init),
)
...
def setup_eval_tasks_and_models(task_base, models_base, hps):
    ...
    tasks, models, hps = jtree.unzip(LDict.of("pert__amp")({
        context_final: (
            ...
            models_base, 
            hps | dict(pert=dict(amp=context_final - hps.pert.context.c_init)),
        )
        for context_final in (
            hps.pert.context.c_init, 
            hps.pert.context.c_final,
        )
    }))
...
ALL_ANALYSES = [
    Effector_ByReplicate(variant=VARIANT),
    Effector_ByReplicate(variant="reach"),
    NetworkActivities_SampleUnits(variant=VARIANT),
    # Perform PCA wrt baseline `reach` variant, and project `steady` variant into that space
]
